...
fact = np.exp(delta_lnA*np.arange(n_bins)) # array of area reduction factors, = [1, 1.64, 2.71, 4.48, 7.39]
...
def regions(butterfly=True, activity_rate=1, cycle_length=1,
        cycle_overlap=0, max_ave_lat=40, min_ave_lat=5, tsim=1200, tstart=0):
    """ 
    ...
    latavg, latrms = active_latitudes(min_ave_lat, max_ave_lat, phase, butterfly=butterfly)
            ...
            for j in lat_bins:
                ...
                if sumv > x: # emerge spot
                    ...
                    if nday > tstart:
                        ...
                        peak_magnetic_flux = Bmax * (flux_distribution_width/width_threshold)**2

                        lat_rad = lat/RAD2DEG
                        lon_rad = lon/RAD2DEG

                        spots[n_count] = {'nday': nday,
