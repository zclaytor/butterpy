...
class Spots(object):
    ...
    def __init__(self, spot_properties, dur=None, alpha_med=0.0001, 
            incl=None, omega=2.0, delta_omega=0.3, 
            diffrot_func=diffrot, decay_timescale=5.0, threshold=0.1):
        ...
        self.emergence_timescale = max(2.0, self.equatorial_period*decay_timescale/5)
        ...
        self.area_max = alpha_med * (peak_magnetic_flux / np.median(peak_magnetic_flux))[keep]
    ...
    def matrix_calc(self, time):
        ...
        cos_beta = np.cos(self.inclination)*np.outer(np.sin(self.latitude), np.ones(N)) \
                   + np.sin(self.inclination)*np.outer(np.cos(self.latitude), np.ones(N))*np.cos(phase)
...
def butterfly(self):
    ...
    fig, ax = subplots()
    ax.scatter(self.spot_properties['nday'], latitude*RAD2DEG, 
